...
# --- KINO TAHRIRLASH UCHUN YANGI KLAVIATURA QO'SHILDI ---
def get_edit_movie_fields_kb() -> InlineKeyboardMarkup:
    """Kino tahrirlash uchun maydonlarni tanlash klaviaturasi"""
    kb = InlineKeyboardBuilder()
    
    # Birinchi qator
    kb.button(text="🎬 Nom", callback_data="edit_title")
    kb.button(text="📝 Janr", callback_data="edit_genre")
    kb.button(text="📄 Tavsif", callback_data="edit_description")
    
    # Ikkinchi qator
    kb.button(text="🔢 Kod", callback_data="edit_code")
    kb.button(text="🗓 Yil", callback_data="edit_year")
    kb.button(text="🌍 Mamlakat", callback_data="edit_country")
    
    # Uchinchi qator
    kb.button(text="⏱ Davomiylik", callback_data="edit_duration")
    kb.button(text="⭐ IMDb", callback_data="edit_imdb_rating")
    kb.button(text="⚙️ Sifat", callback_data="edit_quality")
    
    # To'rtinchi qator (Fayllar)
    kb.button(text="🎥 Fayl (Video/Doc)", callback_data="edit_file_id")
    kb.button(text="🖼 Rasm (Thumbnail)", callback_data="edit_thumbnail_file_id")
    
    # Oxirgi qator (Ortga qaytish)
    kb.button(text="⬅️ Ortga/Qaytish", callback_data="admin_panel_back")
...
def get_confirmation_kb(action: str) -> InlineKeyboardMarkup:
    ...
    kb = InlineKeyboardBuilder()
    # Eslatma: Kino o'chirish uchun action 'delete_movie' bo'ladi.
